...
local function reload_cfg(oldcfg, cfg)
    ...
    for _, key in pairs(ordered_cfg) do
        ...
        if not compare_cfg(val, oldval) then
            ...
            local old_internal_val = box.internal.modified_cfg[key]
            if modify_internal_cfg[key] then
                local status, internal_val =
                    pcall(modify_internal_cfg[key], val)
                if not status then
                    rawset(oldcfg, key, oldval) -- revert the old value
                    return box.error()
                end
                rawset(box.internal.modified_cfg, key, internal_val)
            end
            if not pcall(dynamic_cfg[key]) then
                rawset(box.internal.modified_cfg, key, old_internal_val)
                rawset(oldcfg, key, oldval) -- revert the old value
...
local function load_cfg(cfg)
    ...
    -- Save new box.cfg
    box.cfg = cfg
    box.internal.modified_cfg = {}
    for key, func in pairs(modify_internal_cfg) do
        local status, rc = pcall(func, box.cfg[key])
        if not status then
            box.cfg = locked(load_cfg) -- restore original box.cfg
            box.internal.modified_cfg = {}
            return box.error()
        end
        box.internal.modified_cfg[key] = rc
    end
