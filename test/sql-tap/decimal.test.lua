...
local dec1 = dec.new('-123456789123456789.123456789123456789')
local dec2 = dec.new('9999999999999999999999999999999.9999999')
local dec3 = dec.new('1000')

-- Check that it is possible to create spaces with DECIMAL field.
test:do_execsql_test(
    "dec-1",
    [[
        CREATE TABLE t1 (i INT PRIMARY KEY, d DECIMAL);
        CREATE TABLE t2 (d DECIMAL PRIMARY KEY);
    ]], {
    })
...
test:do_execsql_test(
    "dec-2.2.4",
    [[
        SELECT * FROM t2 ORDER BY d DESC;
    ]], {
        dec2, dec3, dec1
    })
...
test:do_execsql_test(
    "dec-2.3.1",
    [[
        SELECT count(*), d FROM t1 GROUP BY d;
    ]], {
...
test:do_execsql_test(
    "dec-2.3.2",
    [[
        SELECT count(*), d FROM t2 GROUP BY d;
    ]], {
...
test:do_execsql_test(
    "dec-2.6",
    [[
        CREATE VIEW v AS SELECT d FROM t1;
        SELECT * FROM v;
    ]], {
...
--
-- Make sure LIMIT only accepts decimal numbers that can be implicitly cast to
-- INTEGER.
--
test:do_catchsql_test(
    "dec-3.1.1",
    [[
        SELECT 1 LIMIT (SELECT d FROM t1 LIMIT 1);
    ]], {
...
test:do_execsql_test(
    "dec-3.1.2",
    [[
        SELECT 1 LIMIT (SELECT d FROM t1 LIMIT 1 OFFSET 2);
    ]], {
        1
    })

--
-- Make sure OFFSET only accepts decimal numbers that can be implicitly cast to
-- INTEGER.
--
test:do_catchsql_test(
    "dec-3.2.1",
    [[
        SELECT 1 LIMIT 1 OFFSET (SELECT d FROM t1 LIMIT 1);
    ]], {
...
test:do_execsql_test(
    "dec-3.2.1",
    [[
        SELECT 1 LIMIT 1 OFFSET (SELECT d FROM t1 LIMIT 1 OFFSET 2);
    ]], {
    })
