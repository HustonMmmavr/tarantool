...
static
int save_entry_cb(int idx, void *addr, const char *name, size_t offset,
		  void *cb_arg)
{
...
static
int compare_entries(const struct bt_entry *lhs, const struct bt_entry *rhs)
{
...
static
int NOINLINE foo(void *ip_buf[], struct bt_entry entry_buf[])
{
...
static
int NOINLINE bar(void *ip_buf[], struct bt_entry entry_buf[])
{
	note("Calling foo()");
...
static
int NOINLINE baz(int n, void *ip_buf[], struct bt_entry entry_buf[])
{
	int depth = 0;
	if (n <= 0) {
		note("Calling bar()");
